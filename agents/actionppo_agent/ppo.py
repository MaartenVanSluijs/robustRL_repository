...
def ppo(env_name, seed, num_epochs, render, log_results,
    ...
    def get_directory():
        ...
        base_dir = getcwd() + '/agents/actionppo_agent/models/' + env_name + '/'
    ...
    actor = core.MLPActorCritic(env.observation_space, env.action_space, hidden_sizes=[400,300])
    adversary = core.MLPActorCritic(env.observation_space, env.action_space, hidden_sizes=[400,300])

    # Set up experience buffers
    actor_buffer = PPOBuffer(obs_dim, act_dim, EPOCH_LENGTH, gamma, gae_lambda)
    adversary_buffer = PPOBuffer(obs_dim, act_dim, EPOCH_LENGTH, gamma, gae_lambda)

    # Set up optimizers for policy and value functions
    actor_optimizer = Adam(actor.pi.parameters(), lr=PI_LR)
    actor_v_optimizer = Adam(actor.v.parameters(), lr=V_LR)

    adversary_optimizer = Adam(adversary.pi.parameters(), lr=PI_LR)
    adversary_v_optimizer = Adam(adversary.v.parameters(), lr=V_LR)
    ...
    def update(agent_role: str):
        if agent_role == "actor":
            agent = actor
            buffer = actor_buffer
            pi_optimizer = actor_optimizer
            vf_optimizer = actor_v_optimizer
        elif agent_role == "adversary":
            agent = adversary
            buffer = adversary_buffer
            pi_optimizer = adversary_optimizer
            vf_optimizer = adversary_v_optimizer
        ...
        if log_results:
            run.log({"Pi stopping iteration": i})
    ...
    def get_action(action, nu):
        scaled_action = action * (1-alpha)
        scaled_nu = nu * alpha

        combined_action = scaled_action + scaled_nu
        return np.clip(combined_action, -1, 1)
    ...
    for epoch in trange(num_epochs):
        ...
        for t in range(EPOCH_LENGTH):
            ...
            pi, logp_pi, v_pi = actor.step(torch.as_tensor(observation, dtype=torch.float32))
            nu, logp_nu, v_nu = adversary.step(torch.as_tensor(observation, dtype=torch.float32))

            perturbed_action = get_action(pi, nu)

            next_observation, reward, terminated, _, info = env.step(perturbed_action)
            episode_return += reward
            episode_length += 1

            # save and log
            actor_buffer.store(observation, pi, logp_pi, reward, v_pi)
            adversary_buffer.store(observation, nu, logp_nu, -reward, v_nu)
            ...
            if terminated:
                ...
                actor_buffer.finish_path(0)
                adversary_buffer.finish_path(0)
                observation, _ = env.reset()
                episode_return, episode_length = 0, 0

        if not terminated:
            _, v_pi, _ = actor.step(torch.as_tensor(observation, dtype=torch.float32))
            actor_buffer.finish_path(v_pi)

            _, v_nu, _ = adversary.step(torch.as_tensor(observation, dtype=torch.float32))
            adversary_buffer.finish_path(v_nu)
        ...
        if epoch % ratio == 0:
            ...
            # Buffer doesn't always get cleared if ratio != 0, call get to reset pointers
            _ = adversary_buffer.get()
