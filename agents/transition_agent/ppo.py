...
def ppo(env_name,                           # Env Name
        epochs=50, clip_ratio=0.2, gae_lambda=0.97, target_kl=0.01, log_results=False, seed=0, render=False,
        psi_range=0.1, nr_trajectories=500, epsilon=10,
        max_trajectory_length=2000, train_pi_iters=80, train_v_iters=80, gamma=0.99, pi_lr=3e-4,vf_lr=1e-3, 
    ...
    def get_directory():
        ...
        base_dir = getcwd() + '/agents/transition_agent/models/' + env_name + '/'
    ...
    def sample_trajectories() -> list[Trajectory]:
        ...
        for _ in range(nr_trajectories):
            ...
            while not (terminated or truncated):
                ...
                # save and log
                trajectory.store(observation, action, reward, v, logp)
    ...
    for epoch in trange(epochs):
        ...
        trajectories: list[Trajectory] = sample_trajectories()
        ...
        for trajectory in trajectories:
            if trajectory.discounted_return > percentile:
                trajectories.remove(trajectory)

        # Create buffer and populate it with filtered trajectories
        buffer_size = sum([trajectory.size for trajectory in trajectories])
        buffer = PPOBuffer(obs_dim, act_dim, buffer_size, gamma, gae_lambda)
        for trajectory in trajectories:
            for step in range(trajectory.size):
                buffer.store(
                    ...
                    trajectory.val_buf[step],
                    trajectory.logp_buf[step]
                )
            buffer.finish_path(trajectory.trailing_v)
...
if __name__ == '__main__':
    ...
    ppo(args.env_name, actor_critic=core.MLPActorCritic, ac_kwargs=dict(hidden_sizes=[400,300]), 
        seed=args.seed, epochs=args.num_epochs, log_results=args.log_results, render=args.render, 
