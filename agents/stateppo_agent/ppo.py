...
def ppo(env_name, seed, num_epochs, render, log_results,
        ...
        target_eps, reg_factor,
        train_pi_iters=80, train_v_iters=80, SGLD_steps=5):
    ...
    def get_directory():
        ...
        base_dir = getcwd() + '/agents/stateppo_agent/models/' + env_name + '/'
    ...
    def compute_loss_pi(data):
        ...
        policy_regularizer = compute_regularizer(obs, act)

        return loss_pi + policy_regularizer, pi_info
    ...
    def compute_regularizer(observation, action):
        ...
        step_eps = torch.tensor(robust_eps / SGLD_steps, dtype=torch.float32)

        # Compute the logp_a from the nominal observation, go through pi for the gradient
        with torch.no_grad():
            old_distribution = ac.pi._distribution(observation)
            logp_old = ac.pi._log_prob_from_distribution(old_distribution, action)
        ...
        for i in range(SGLD_steps):
            ...
            new_distribution = ac.pi._distribution(adv_phi)
            logp_a = ac.pi._log_prob_from_distribution(new_distribution, tensor(ac.act(adv_phi)))
            adv_loss = (logp_old-logp_a).mean()
        ...
        with torch.no_grad():
            new_distribution = ac.pi._distribution(adv_phi)
            logp_a = ac.pi._log_prob_from_distribution(new_distribution, tensor(ac.act(adv_phi)))
        action_reg_loss = (logp_old-logp_a).mean()
